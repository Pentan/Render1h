...
public:
    ...
    void getNextRays(const Ray &ray, const Hitpoint &hp, const int depth, Random *rnd, std::vector<TraceInfo> &outvecs) {
        const Ray reflection_ray = Ray(hp.position_, ray.dir_ - hp.normal_ * 2.0 * dot(hp.normal_, ray.dir_));
        const Vec orienting_normal = hp.orientingNormal(ray);
        const bool into = dot(hp.normal_, orienting_normal) > 0.0;
        ...
        const Ray refraction_ray = Ray(
            hp.position_,
            normalize(ray.dir_ * nnt - hp.normal_ * (into ? 1.0 : -1.0) * (ddn * nnt + sqrt(cos2t)))
            );
        ...
        if(depth > 2) {
            ...
            if(rnd->next01() < probability) { // reflection
                nxttrc.ray = reflection_ray;
                nxttrc.weight = Vec(Re / (1.0 - probability));
                /*
                ...
                */
                ...
                nxttrc.weight = Vec(Tr / (1.0 - probability));
                /*
