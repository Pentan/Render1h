...
///// base class
Vec Texture::applyTransform(const Vec &p) const {
    if(!isUseTrans) {
        return p;
    }
    return Mat4::transformV3(transform, p);
}
...
Color NoiseTexture::sample(const Vec &p) const {
    Vec lp = applyTransform(p);
...
Color VoronoiTexture::sample(const Vec &p) const {
    // local point
    Vec lp = applyTransform(p);
    // stepping point
    Vec sp = Vec(floor(lp.x), floor(lp.y), floor(lp.z));
    ...
    for(int iz = -1; iz <= 1; iz++) {
        for(int iy = -1; iy <= 1; iy++) {
            for(int ix = -1; ix <= 1; ix++) {
                ...
                Vec pp = (csp + jv) - lp;
                double tmpll = Vec::dot(pp, pp); // square len
                if(tmpll < minll) {
                    minll = tmpll;
                }
            }
    ...
    //return Vec(1.0 - sqrt(minll));
    return Vec(sqrt(minll));
}
...
Color FractalNoiseTexture::sample(const Vec &p) const {
    Vec lp = applyTransform(p);
    Color ret = Color(0.0);
    double weight = 1.0;
    double wtotal = 0.0;
